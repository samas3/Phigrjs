...
function pushLineEvent(ls, le) {
	const { startTime, endTime, start, end, easingType = 1, easingLeft = 0, easingRight = 1, bezier = 0, bezierPoints = [0, 0, 0, 0] } = le;
	...
	if (bezier == 0) {
		...
		if (start === end) ls.push({ startTime, endTime, start, end, delta });
		const bezier = new BezierEasing([bezierPoints[0], bezierPoints[1]], [bezierPoints[2], bezierPoints[3]]);
		for (let i = startTime; i < endTime; i++) {
			let st = (i - startTime) / (endTime - startTime);
			let et = (i - startTime + 1) / (endTime - startTime);
            let t1 = bezier.getValue(st), t2 = bezier.getValue(et);
            let val1 = t1 * (end - start) + start, val2 = t2 * (end - start) + start;
			ls.push({ startTime: i, endTime: i + 1, start: val1, end: val2, delta: 0 });
		}
	}
}
...
class EventLayer {
	...
	pushMoveXEvent(startTime, endTime, start, end, easingType, easingLeft, easingRight, bezier, bezierPoints) {
		this.moveXEvents.push({ startTime, endTime, start, end, easingType, easingLeft, easingRight, bezier, bezierPoints });
	}
	pushMoveYEvent(startTime, endTime, start, end, easingType, easingLeft, easingRight, bezier, bezierPoints) {
		this.moveYEvents.push({ startTime, endTime, start, end, easingType, easingLeft, easingRight, bezier, bezierPoints });
	}
	pushRotateEvent(startTime, endTime, start, end, easingType, easingLeft, easingRight, bezier, bezierPoints) {
		this.rotateEvents.push({ startTime, endTime, start, end, easingType, easingLeft, easingRight, bezier, bezierPoints });
	}
	pushAlphaEvent(startTime, endTime, start, end, easingType, easingLeft, easingRight, bezier, bezierPoints) {
		this.alphaEvents.push({ startTime, endTime, start, end, easingType, easingLeft, easingRight, bezier, bezierPoints });
	}
...
class ExtendedEvent {
	...
	pushColorEvent(startTime, endTime, start, end, easingType, easingLeft, easingRight, bezier, bezierPoints) {
		this.colorEvents.push({ startTime, endTime, start, end, easingType, easingLeft, easingRight, bezier, bezierPoints });
	}
	pushTextEvent(startTime, endTime, start, end) {
		this.textEvents.push({ startTime, endTime, start, end });
	}
	pushScaleXEvent(startTime, endTime, start, end, easingType, easingLeft, easingRight, bezier, bezierPoints) {
		this.scaleXEvents.push({ startTime, endTime, start, end, easingType, easingLeft, easingRight, bezier, bezierPoints });
	}
	pushScaleYEvent(startTime, endTime, start, end, easingType, easingLeft, easingRight, bezier, bezierPoints) {
		this.scaleYEvents.push({ startTime, endTime, start, end, easingType, easingLeft, easingRight, bezier, bezierPoints });
	}
}
class LineRPE {
	constructor(bpm) {
	...
	pushNote(type, time, positionX, holdTime, speed, isAbove, isFake, size, alpha, visibleTime) {
	...
	format() {
		...
		for (const i of this.extendedEvents.colorEvents) {
			result.colorEvents.push({
				...
				bezier: i.bezier || 0,
				bezierPoints: i.bezierPoints || [0, 0, 0, 0]
			});
		}
		...
		for (const i of this.extendedEvents.scaleXEvents) {
			result.scaleXEvents.push({
				startTime: i.startTime,
				endTime: i.endTime,
				start: i.start,
				end: i.end,
		...
		for (const i of this.extendedEvents.scaleYEvents) {
			result.scaleYEvents.push({
				startTime: i.startTime,
				endTime: i.endTime,
				start: i.start,
				end: i.end,
...
function parseRPE(pec, filename) {
	...
	for (const i of data.judgeLineList) {
		...
		// TODO: isCover, Image, zOrder, attachUI
		...
		for (const e of i.eventLayers) {
			...
			for (const j of (e.moveXEvents || [])) {
				...
				layer.pushMoveXEvent(startTime, endTime, j.start, j.end, j.easingType, j.easingLeft, j.easingRight, j.bezier, j.bezierPoints);
			}
			for (const j of (e.moveYEvents || [])) {
				if (j.linkgroup === undefined) j.linkgroup = 0;
				const startTime = bpmList.calc(j.startTime[0] + j.startTime[1] / j.startTime[2]);
				const endTime = bpmList.calc(j.endTime[0] + j.endTime[1] / j.endTime[2]);
				layer.pushMoveYEvent(startTime, endTime, j.start, j.end, j.easingType, j.easingLeft, j.easingRight, j.bezier, j.bezierPoints);
			}
			for (const j of (e.rotateEvents || [])) {
				if (j.linkgroup === undefined) j.linkgroup = 0;
				const startTime = bpmList.calc(j.startTime[0] + j.startTime[1] / j.startTime[2]);
				const endTime = bpmList.calc(j.endTime[0] + j.endTime[1] / j.endTime[2]);
				layer.pushRotateEvent(startTime, endTime, j.start, j.end, j.easingType, j.easingLeft, j.easingRight, j.bezier, j.bezierPoints);
			}
			for (const j of (e.alphaEvents || [])) {
				if (j.linkgroup === undefined) j.linkgroup = 0;
				const startTime = bpmList.calc(j.startTime[0] + j.startTime[1] / j.startTime[2]);
				const endTime = bpmList.calc(j.endTime[0] + j.endTime[1] / j.endTime[2]);
				layer.pushAlphaEvent(startTime, endTime, j.start, j.end, j.easingType, j.easingLeft, j.easingRight, j.bezier, j.bezierPoints);
			}
		...
		for (const j of (i.extended.colorEvents || [])) {
			const startTime = bpmList.calc(j.startTime[0] + j.startTime[1] / j.startTime[2]);
			const endTime = bpmList.calc(j.endTime[0] + j.endTime[1] / j.endTime[2]);
			extended.pushColorEvent(startTime, endTime, j.start, j.end, j.easingType, j.easingLeft, j.easingRight, j.bezier, j.bezierPoints);
		}
		...
		for (const j of (i.extended.scaleXEvents || [])) {
			const startTime = bpmList.calc(j.startTime[0] + j.startTime[1] / j.startTime[2]);
			const endTime = bpmList.calc(j.endTime[0] + j.endTime[1] / j.endTime[2]);
			extended.pushScaleXEvent(startTime, endTime, j.start, j.end, j.easingType, j.easingLeft, j.easingRight, j.bezierPoints);
		}
		for (const j of (i.extended.scaleYEvents || [])) {
			const startTime = bpmList.calc(j.startTime[0] + j.startTime[1] / j.startTime[2]);
			const endTime = bpmList.calc(j.endTime[0] + j.endTime[1] / j.endTime[2]);
			extended.pushScaleYEvent(startTime, endTime, j.start, j.end, j.easingType, j.easingLeft, j.easingRight, j.bezierPoints);
		}
